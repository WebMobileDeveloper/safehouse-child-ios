...
class AppDelegate: UIResponder, UIApplicationDelegate, CLLocationManagerDelegate {
    ...
    func showAddTextView(originText: String){
        if let currentViewController = self.window?.rootViewController {
            let mVC = UIStoryboard(name: "Map", bundle: nil).instantiateViewController(withIdentifier: "addTextViewController") as! addTextViewController
            mVC.originText = originText
            mVC.modalPresentationStyle = .overCurrentContext
            currentViewController.present(mVC, animated: true, completion: nil)
        }
    }
    func showRecordingView(){
        if let currentViewController = self.window?.rootViewController {
            let mVC = UIStoryboard(name: "Map", bundle: nil).instantiateViewController(withIdentifier: "recordingViewController") as! recordingViewController
            mVC.modalPresentationStyle = .overCurrentContext
            currentViewController.present(mVC, animated: true, completion: nil)
        }
...
// MARK:- Location magager functions   &&&&&&&&&&&&&&&&&&
    ...
    func submitCheckIn(target: UIViewController, checkin_id:String, photo_url:String, photo_text:String, completionHandler: @escaping ()->Void = {}) {
        if let currentUser = Auth.auth().currentUser{
            currentUser.getIDTokenForcingRefresh(true, completion: { (idToken, error) in
                if error != nil{
                    showAlert(target: target, message: "Network connection failed.", title: "Error", hander: {
                        completionHandler()
                    })
                }
                let headers:HTTPHeaders = [
                ...
                let url = "https://us-central1-safehouse-488e5.cloudfunctions.net/checkin"
                Alamofire.request(url,
                                  method: HTTPMethod.post,
                                  parameters: parameters,
                                  encoding: JSONEncoding.default,
    ...
    func sendEmergencyRequest(target: UIViewController, audio_url:String, completionHandler: @escaping ()->Void = {}) {
        if let currentUser = Auth.auth().currentUser{
            currentUser.getIDTokenForcingRefresh(true, completion: { (idToken, error) in
                if error != nil{
                    showAlert(target: target, message: "Network connection failed.", title: "Error", hander: {
                ...
                let url = "https://us-central1-safehouse-488e5.cloudfunctions.net/emergency"
                Alamofire.request(url,
                                  method: HTTPMethod.post,
                                  parameters: parameters,
                                  encoding: JSONEncoding.default,
